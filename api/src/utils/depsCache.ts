...
export function localNodeModulesDir(deploymentId: string): string {
  return path.join(getDeploymentCacheDir(deploymentId), "node_modules");
}
...
export function getNodeModulesContainerPath(): string {
...
export async function ensureNodeModulesDir(deploymentId: string): Promise<string> {
  const dir = localNodeModulesDir(deploymentId);
  await fs.promises.mkdir(dir, { recursive: true });
  return dir;
}
...
async function nodeModulesCacheReady(
  deploymentId: string,
  lockfileInfo: { hash: string; file: string },
  projectRootRel: string
): Promise<boolean> {
  ...
  if (!(await cacheDirHasEntries(nmDir))) return false;

  // Sanity check: empty mount dir is not a real install tree.
  return (
    fs.existsSync(path.join(nmDir, ".bin")) ||
...
async function saveLocalNpmCache(
  ...
  logger.log("Dependency cache metadata updated (npm cache + node_modules on disk)");
}
...
/** Check local mounts, then optional R2 npm cache. */
export async function restoreDepsCache(
  ...
  const skipInstall = await nodeModulesCacheReady(
    deploymentId,
    lockfileInfo,
    projectRootRel
  );
  ...
  if (skipInstall) {
    logger.log("Mounting cached node_modules into container...");
    logger.success(
      `Skipping install — ${lockfileInfo.file} unchanged, reusing node_modules`
    );
  }

  return { preferOffline, skipInstall };
}
